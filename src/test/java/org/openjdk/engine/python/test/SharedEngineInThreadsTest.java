...
public class SharedEngineInThreadsTest {

    private static final int NUM_THREADS = 25;

    @Test
    public void testSharedEngineInManyThreads() throws ScriptException, InterruptedException {
        var m = new ScriptEngineManager();
        try (var e = (PythonScriptEngine) m.getEngineByName("python")) {
            IO.println(Thread.currentThread());
            IO.println("engine created " + e + ", isMainEngine? " + e.isMainEngine());
                    ...
                    try {
                        e.withPyThreadState(() -> {
                            try {
                                ...
                                IO.println("leaving main engine usage in thread " + j);
                            } catch (Throwable th) {
